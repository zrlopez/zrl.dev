...
import { BASE_KPIS, SEED_DATA, type Kpis } from '@/app/projects/annotation-dashboard/_components/dashboardData';
...
describe('BASE_KPIS', () => {
  ...
  it('change strings are non-empty and contain %', () => {
    const changeKeys: (keyof Kpis)[] = [
      ...
      'teamEfficiencyChange', 'capacityUtilizationChange',
    ];
...
describe('SEED_DATA.throughputData', () => {
  ...
  it('byProcess entries sum to approximately 100%', () => {
    const total = SEED_DATA.throughputData.byProcess.reduce((s, p) => s + p.percentage, 0);

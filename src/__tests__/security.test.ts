/**
 * security.test.ts
 * Unit tests for src/lib/security.ts — full branch coverage.
 */

jest.mock('server-only', () => ({}));
jest.mock('html-escaper', () => ({ escape: (s: string) => s.replace(/&/g,'&amp;').replace(/</g,'&lt;').replace(/>/g,'&gt;').replace(/"/g,'&quot;').replace(/'/g,'&#39;') }));

import { sanitizeField, sanitizeHeaderField, htmlEscape, validateEmail } from '@/lib/security';

...
import type { Kpis, SEED_DATA } from "../dashboardData";

interface Props { kpis: Kpis; isDark: boolean; data: typeof SEED_DATA; }
...
export function AlertsTab({ kpis, isDark, data }: Props) {
  ...
  const liveThresholds = data.alertsData.thresholds.map((t) => {
    if (t.metric === "Error Rate")           return { ...t, current: kpis.errorRate,           status: kpis.errorRate           >= t.threshold ? "warning" : "ok" };
    if (t.metric === "Capacity Utilization") return { ...t, current: kpis.capacityUtilization, status: kpis.capacityUtilization >= t.threshold ? "warning" : "ok" };
    return t;
  });
  ...
  return (
    <div className="space-y-4">
            ...
            <div key={alert.id} className={`rounded-lg border p-3 ${alert.priority === "high" ? "border-red-200 dark:border-red-800 bg-red-50 dark:bg-red-900/10" : "border-amber-200 dark:border-amber-800 bg-amber-50 dark:bg-amber-900/10"}`}>
              <div className="flex items-start justify-between gap-2">
                ...
                <div className="flex items-center gap-1.5 shrink-0">
                  <span className={`rounded px-2 py-0.5 text-xs font-semibold ${SEVERITY_CLASSES[alert.priority]}`}>{alert.priority}</span>
                  <span className={`rounded px-2 py-0.5 text-xs font-semibold ${STATUS_CLASSES[alert.status]}`}>{alert.status}</span>
                </div>
              </div>
            </div>
          ))}
      ...
      <div className="grid grid-cols-1 gap-4 lg:grid-cols-2">
        <DashboardChartCard title="Alert History" subtitle="Last 5 days">
          <ResponsiveContainer width="100%" height={220}>
            <BarChart data={data.alertsData.history}>
              ...
              <Bar dataKey="pending"  fill={CHART_COLORS[1]} name="Pending"  />
            </BarChart>
          </ResponsiveContainer>
        </DashboardChartCard>

        <DashboardChartCard title="Alert Thresholds" subtitle="Live values vs limits">
              ...
              <div key={t.metric} className="space-y-1">
                <div className="flex items-center justify-between text-xs">
                  ...
                  <div className="flex items-center gap-2">
                    <span className="text-muted-foreground tabular-nums">{t.current} / {t.threshold}</span>
                    <span className={`rounded px-2 py-0.5 font-semibold ${SEVERITY_CLASSES[t.status]}`}>{t.status}</span>
                  </div>
                </div>
                <div className="h-2 w-full rounded-full bg-muted overflow-hidden">
                  <div className="h-full rounded-full transition-all duration-500" style={{ width: `${Math.min(100, (t.current / t.threshold) * 100)}%`, backgroundColor: t.status === "warning" ? CHART_COLORS[2] : CHART_COLORS[0] }} />
                </div>
              </div>
            ))}
          </div>

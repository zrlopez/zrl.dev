export const humansText = ` ______
/\\___  \\
\\/_/  /__
  /\\_____\\
  \\/_____/

The humans.txt file explains the person, stack,
and craft choices behind this site: humanstxt.org.

_______________________________________________________________________________

HUMAN | ヒューマン

Zachary Ryan Lopez
AI/ML Data Operations Analyst
Austin, TX, USA

Site: https://zrl.dev/contact
LinkedIn: https://linkedin.com/in/zrlopez
GitHub: https://github.com/zrlopez
Kaggle: https://kaggle.com/zrlopez

Focus:
- AI/ML data operations
- Annotation quality systems
- Incident response and observability
- Python, SQL, dashboards, and practical web tooling

_______________________________________________________________________________

TECHNOLOGY | テクノロジー

Infrastructure:
- Cloudflare
- Vercel
- GitHub

Frontend:
- Remix
- React
- Three.js
- Framer Motion

Project tooling:
- Next.js 15
- TypeScript
- Recharts
- Pandas
- SQLite
- FastAPI

_______________________________________________________________________________

FONTS | フォント

Primary:
- Gotham
- IPA Gothic

Support:
- Hiragino Kaku Gothic
- Geist Mono

_______________________________________________________________________________

STANDARDS | スタンダード

Web:
- HTML5
- CSS3
- JavaScript

Application:
- TypeScript
- Accessibility-minded responsive design
- Static asset optimization

_______________________________________________________________________________

SITE | サイト

Purpose:
- Portfolio for an AI/ML Data Operations Analyst focused on systems thinking and selected builds

Current branch:
- Remix revamp of zrl.dev

Last update:
- 2026/08/30

Language:
- English

_______________________________________________________________________________

COLOPHON | コロフォン

Visual direction:
- Harvard Crimson #A51C30
- Gotham with IPA Gothic accents

Build character:
- Remix / Vite / Three.js
- Cloudflare and Vercel delivery
- Vim bindings everywhere

_______________________________________________________________________________

CREDITS | クレジット

Original portfolio foundation:
- Hamish Williams portfolio (MIT)

This version:
- Custom content, rewritten project narratives, refreshed imagery, and ML/AI ops framing by Zachary Ryan Lopez
`;
